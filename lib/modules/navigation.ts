...
export const NAVIGATION_REGISTRY: readonly NavigationGroup[] = [
  ...
  { id: "finance", label: "회계·정산", items: [
    ...
    { id: "finance-settlements", label: "직원 정산", route: "/finance/settlements", icon: "₩", moduleId: "finance", match: "prefix" },
    ...[["expenses", "운영비"], ["profit-loss", "월 손익"], ["cash-flow", "현금흐름"], ["closing", "월 마감"]].map(([id, label]) => ({ id: `finance-${id}`, label, route: null, icon: "₩", moduleId: "finance" as const, roles: ADMIN_ROLES, status: "coming_soon" as const })),
  ] },
  ...
  return NAVIGATION_REGISTRY.map((group) => ({
    ...
    items: group.items.filter((item) => {
      const roleAllowed =
        ...
        (companyRole != null && item.companyRoles?.includes(companyRole) === true);

      return roleAllowed && (!item.featureFlag || featureFlags.has(item.featureFlag));
    }),
