    ...
    <div className="space-y-6">
      {feedback ? (
        <div
          className={`rounded-xl border px-4 py-3 text-sm font-medium ${
            ...
            feedback.includes("필수") ||
            feedback.includes("중복")
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs leading-relaxed text-slate-600">
              현장을 선택해 금액만 먼저 등록할 수 있습니다. 영수증·거래명세서가 있으면 촬영하거나 첨부해 자동인식하고, 틀린 부분만 고치면 됩니다.
            </p>
          </div>
          ...
          <form action={action} className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            ...
            {isSelectedSettled ? (
              <div className="sm:col-span-2 xl:col-span-3 rounded-xl border border-orange-300 bg-orange-50 px-4 py-3 text-sm text-orange-950">
                <p className="font-bold">
                  정산완료 현장입니다. 이 지출은 자동으로 사후지출로 기록됩니다.
                </p>
                <p className="mt-1 text-xs leading-relaxed">
                  직원은 평소처럼 제출하면 됩니다. 실제 현장손익에는 반영되고,
                  회사부담/다음 정산 차감/회수 여부는 관리자가 승인할 때 결정합니다.
                </p>
              </div>
            ) : null}
            ...
            <div className="sm:col-span-2 xl:col-span-3 rounded-xl border border-sky-200 bg-sky-50/60 p-4">
              <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <label>
                  ...
                  <select
                    ...
                    className="min-h-11 w-full rounded-lg border border-gray-300 bg-white px-3 text-sm"
                  >
                    {Object.entries(EXPENSE_DOCUMENT_LABELS).map(([value, label]) => (
                ...
                <div className="flex items-end">
                  <button
                    ...
                    className="min-h-11 w-full rounded-lg bg-sky-700 px-4 text-sm font-bold text-white hover:bg-sky-800 disabled:opacity-50"
                  >
              ...
              <p className="mt-2 text-xs text-slate-600">
                증빙은 선택사항입니다. 파일이 있으면 즉시 자동인식하고, 없으면 금액만 먼저 등록한 뒤 관리자가 나중에 영수증·카드내역을 보완할 수 있습니다. 같은 파일은 중복 등록을 차단합니다.
              </p>
            </div>
            ...
            <label>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  const found = vendors.find(
                    (vendor) => normalizeVendor(vendor.name) === norm,
                  );
                  setVendorId(found?.id ?? "");
                }}
                placeholder="상호명 입력 또는 자동인식"
            ...
            <label>
              ...
              {paymentMethod === "company_card" ? (
                <span className="mt-1 block text-[11px] font-semibold text-sky-700">
                  법인카드는 영수증 없이도 등록할 수 있습니다. 카드 승인문자·법인카드 내역으로 확인하고 영수증은 추후 보완할 수 있습니다.
                </span>
              ) : null}
            </label>
            <label>
            ...
            <label>
              ...
              <input
                ...
                required
                className={`min-h-11 w-full rounded-lg border px-3 text-sm font-bold ${
              ...
              />
              {supplyAmount + vatAmount !== totalAmount ? (
                <span className="mt-1 block text-[11px] font-bold text-red-700">
                  공급가 + 부가세가 합계와 다릅니다.
                </span>
              ) : null}
            </label>
            ...
            <div className="flex items-end">
              ...
              <button
                ...
                className="min-h-11 w-full rounded-lg bg-navy-900 px-4 text-sm font-bold text-white hover:bg-navy-800 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {submitting
                  ? "제출 중..."
                  : isFinanceAdmin
            ...
            {pendingVendors.map((vendor) => (
              <div key={vendor.id} className="rounded-lg border border-violet-200 bg-white p-3">
                ...
                <p className="mt-1 text-xs text-slate-600">
                  {vendor.business_number || "사업자번호 미입력"}
                </p>
                <button
                  disabled={pending}
      ...
      {isFinanceAdmin ? (
        <section className="rounded-xl border border-amber-200 bg-amber-50/60 p-4 sm:p-5">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h2 className="font-bold text-amber-950">지출 승인대기</h2>
              <p className="mt-1 text-xs text-amber-800">
                일반 지출은 바로 승인하고, 사후지출은 처리방법까지 정한 뒤 승인합니다.
              </p>
            </div>
          ...
          {pendingRequests.length === 0 ? (
            <p className="mt-4 rounded-lg bg-white/70 px-4 py-5 text-center text-sm text-amber-900">
              승인대기 지출이 없습니다.
            </p>
          ) : (
                  ...
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-slate-700">
                        {row.vendor_name_snapshot || "거래처 미지정"} · {EXPENSE_CATEGORY_LABELS[row.category]} · {row.description}
                      </p>
        ...
        <section className="rounded-xl border border-emerald-200 bg-emerald-50/60 p-4 sm:p-5">
          ...
          <div className="mt-3 space-y-3">
            {approvedRequests.map((row) => (
              <div
                key={row.id}
                className="flex flex-col gap-3 rounded-lg border border-emerald-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between"
              >
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <p className="font-bold text-slate-950">
                  ...
                  {row.is_post_settlement && row.post_settlement_treatment ? (
                    <p className="mt-1 text-xs font-semibold text-orange-800">
                      {POST_SETTLEMENT_TREATMENT_LABELS[row.post_settlement_treatment]}
                      {row.settlement_adjustment_amount > 0
                        ? ` · 정산조정 ${money(row.settlement_adjustment_amount)}`
                        : ""}
                    </p>
                  ) : null}
                </div>
        ...
        <section className="rounded-xl border border-indigo-200 bg-indigo-50/60 p-4 sm:p-5">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-indigo-800">
                사후지출 때문에 이전 정산에서 조정할 금액입니다. 다른 현장 손익에는 섞지 않고 다음 직원 정산에서 별도 차감합니다.
              </p>
            </div>
            <span className="rounded-full bg-indigo-100 px-2.5 py-1 text-xs font-bold text-indigo-900">
              {activeAdjustments.length}건
            </span>
          </div>
          <div className="mt-3 space-y-2">
            {activeAdjustments.map((item) => (
              <div
                key={item.id}
                className="rounded-lg border border-indigo-200 bg-white px-4 py-3 text-sm"
              >
                ...
                <p className="mt-1 text-xs text-slate-600">
                  {item.reason ||
                    item.source_expense?.description ||
                    "사후지출 정산조정"}
                </p>
              </div>
            ))}
      ...
      <section className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="border-b border-gray-100 px-4 py-4 sm:px-5">
          <h2 className="font-bold text-slate-950">지출 이력</h2>
          <p className="mt-1 text-xs text-slate-600">
            승인·지급·반려·취소 이력을 보존하고 사후지출은 별도로 표시합니다.
          </p>
        </div>
                ...
                <tr key={row.id} className="border-t border-gray-100">
                  ...
                  <td className="px-4 py-3">
                    {(row.expense_documents?.length ?? 0) > 0 ? (
                      ...
                      <span className="font-semibold text-amber-700">미첨부</span>
                    )}
                  </td>
...
function RejectButton({
  ...
  pending: boolean;
  run: (fn: () => Promise<ExpenseActionResult>) => void;
}) {
  return (
    <button
      disabled={pending}
      ...
      className="min-h-11 rounded-lg border border-red-200 px-4 text-sm font-semibold text-red-700"
    >
      반려
    </button>
  );
  ...
  return (
    <div className="mt-4 rounded-xl border border-orange-200 bg-orange-50/70 p-4">
      ...
      <p className="mt-1 text-xs leading-relaxed text-orange-800">
        원래 현장의 실제손익에는 이 비용 전체가 반영됩니다. 아래 선택은 직원정산·회수 처리방법만 결정합니다.
      </p>
      <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        <label>
          <span className="mb-1 block text-xs font-semibold text-slate-700">발생 사유</span>
          <select
            value={reason}
            onChange={(e) => setReason(e.target.value as PostSettlementReason)}
            className="min-h-11 w-full rounded-lg border border-orange-200 bg-white px-3 text-sm"
          >
        ...
        <label>
          <span className="mb-1 block text-xs font-semibold text-slate-700">처리 방법</span>
          <select
            value={treatment}
            onChange={(e) => setTreatment(e.target.value as PostSettlementTreatment)}
            className="min-h-11 w-full rounded-lg border border-orange-200 bg-white px-3 text-sm"
          >
        ...
        {treatment === "next_settlement_deduction" ? (
          <>
            <label>
              <span className="mb-1 block text-xs font-semibold text-slate-700">차감 대상 직원</span>
              <select
                value={employeeId}
            ...
            <label>
              <span className="mb-1 block text-xs font-semibold text-slate-700">다음 정산 차감액</span>
              <input
                type="number"
                min="1"
              ...
              <div className="mt-1 flex gap-1">
                <button
                  type="button"
                  onClick={() => setAdjustmentAmount(Math.round(row.total_amount * 0.5))}
                  className="rounded bg-white px-2 py-1 text-[11px] font-semibold text-orange-800 ring-1 ring-orange-200"
                >
                  50% 계산
                </button>
                <button
        ...
        {treatment === "vendor_recovery" || treatment === "customer_rebill" ? (
          <label>
            <span className="mb-1 block text-xs font-semibold text-slate-700">회수/청구 예정액</span>
            <input
              type="number"
              min="1"
              max={row.total_amount}
            ...
            <button
              type="button"
              onClick={() => setRecoveryAmount(row.total_amount)}
              className="mt-1 rounded bg-white px-2 py-1 text-[11px] font-semibold text-orange-800 ring-1 ring-orange-200"
            >
              전액 입력
            </button>
          </label>
        ...
        <label className="sm:col-span-2 lg:col-span-3">
          <span className="mb-1 block text-xs font-semibold text-slate-700">관리 메모</span>
          <input
            value={note}
            onChange={(e) => setNote(e.target.value)}
            placeholder="필요한 경우 귀책·회수 계획 등을 기록"
            className="min-h-11 w-full rounded-lg border border-orange-200 bg-white px-3 text-sm"
          />
        </label>
      </div>
      <div className="mt-3 flex flex-wrap gap-2">
        <button
          ...
          className="min-h-11 rounded-lg bg-orange-700 px-4 text-sm font-bold text-white disabled:opacity-50"
        >
          처리방법 저장 + 승인
        </button>
        <button
          ...
          disabled={pending}
          onClick={onReject}
          className="min-h-11 rounded-lg border border-red-200 bg-white px-4 text-sm font-semibold text-red-700"
        >
          반려
        </button>
      </div>
  ...
  return (
    <div
      className={`rounded-xl border p-4 ${
        tone === "warn" ? "border-orange-200 bg-orange-50" : "border-gray-200 bg-white"
      }`}
    >
