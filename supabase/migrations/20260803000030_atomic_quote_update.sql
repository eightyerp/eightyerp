-- =============================================================================
-- Eighty ERP — 견적 헤더+항목 원자적 수정 RPC
-- 파일: 20260803000030_atomic_quote_update.sql
--
-- 목적:
--   - quotes 헤더 UPDATE 와 quote_items 동기화를 단일 트랜잭션으로 처리
--   - 항목 실패 시 헤더만 남는 부분 저장 방지
--   - cost_type UI 별칭 `자재+시공` → DB canonical `시공+자재` 정규화
--   - migration 29 sync_quote_items 의 ID 완전성·명시 soft-delete 정책 재사용
--
-- 안전:
--   - DROP TABLE / DELETE FROM / TRUNCATE / 기존 행 일괄 backfill 없음
--   - 함수 CREATE OR REPLACE + GRANT/REVOKE 만 수행
--   - SQL DELETE 미사용 (항목 제거는 deleted_at UPDATE)
--   - company_id / created_by / quote_number / customer_id 는 클라이언트가 변경 불가
--
-- 적용 순서: migration 27 → 28 → 29 → 본 파일(30)
-- 재실행: create or replace / revoke·grant 재적용 가능
-- 이번 배포 단계에서 파일만 추가. 운영 적용은 별도 승인 후.
-- =============================================================================
...
begin
  ...
  -- 5) 헤더 UPDATE (quote_number/customer_id/company_id/created_by 변경 금지)
  ...
  -- 6) 기존 항목 batch UPDATE (cost_type 정규화 포함)
  ...
  -- 7) 신규 항목 set-based INSERT (id 없음 → 테이블 DEFAULT/gen_random_uuid)
  --    quote_id/company_id 는 검증된 서버 변수만 사용. INSERT 전 검증 완료.
  ...
  -- 8) 명시된 removed ID만 batch soft-delete
...
comment on function public.update_quote_with_items(uuid, jsonb, jsonb, uuid[]) is
  '견적 헤더+항목 원자적 수정. cost_type 정규화, ID 완전성 검증, 실패 시 전체 rollback.';
