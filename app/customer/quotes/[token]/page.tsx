...
import QuoteCustomerShareView from "@/components/quotes/QuoteCustomerShareView";
import {
...
export default async function CustomerQuoteSharePage({
  ...
  const documentModel: QuoteDocumentModel = {
    ...
    specialDiscountMemo: share.special_discount_memo ?? null,
  ...
  const files = (share.files ?? []).map((f) => ({
    ...
    signedUrl: signedUrls[f.id],
  }));

  return (
    <QuoteCustomerShareView
      model={documentModel}
      files={files}
      primaryPdfId={primaryPdf?.id ?? null}
    />
  );
}
