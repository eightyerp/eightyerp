...
const companyScopeGuardMigration = fs.readFileSync(
...
for (const role of ["owner", "director", "admin"]) {
  ...
  assert.match(companyScopeGuardMigration, new RegExp(`v_(?:actor_)?company_role not in \\([^)]*['"]${role}['"]`), `RPC missing ${role}`);
}
...
assert.doesNotMatch(
  assignmentGuardMigration,
  /\b(?:delete\s+from|truncate|drop\s+(?:table|function))\b/i,
  "assignment guard migration must not delete data or drop schema objects",
);
assert.match(
  staffApprovals,
