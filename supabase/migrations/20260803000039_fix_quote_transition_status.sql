-- =============================================================================
-- Eighty ERP — Bundle 1 보완: 전환 허용 상태를 실 운영 흐름에 맞춤
-- 파일: 20260803000039_fix_quote_transition_status.sql
--
-- 배경:
--   Migration 38 은 ERP_QUOTE_STATUSES 의 '승인'·'계약전환' 을 전환 전제 상태로
--   사용했다. 그러나 실제 ERP 견적 흐름은
--     작성중 → markQuoteSent → 발송완료
--   이며, 견적 고객 '승인' 절차는 구현되어 있지 않다.
--   setContractQuote 는 contracts 없이 is_contract_quote + status='계약전환' 만
--   설정하는 표시용 기능이다.
--
-- 변경 (최소):
--   - transition_quote_to_contract 상태 가드만 교체
--   - 허용: '발송완료'
--   - 거부: '작성중'(draft) 및 그 외 (승인·계약전환을 전환 전 필수로 요구하지 않음)
--   - 전환 완료 기준은 계속 public.contracts.quote_id 존재
--   - 전환 성공 후 표시용으로 status='계약전환', is_contract_quote=true 유지
--
-- 유지:
--   - RPC 이름·매개변수·반환형 동일
--   - SECURITY DEFINER + search_path=public
--   - 원자성, quote_id UNIQUE 멱등, RLS, 견적 잠금 trigger 구조
--
-- 안전:
--   - Migration 38 파일 미수정
--   - 기존 행 UPDATE/DELETE/TRUNCATE/백필 없음
--   - DROP TABLE / DROP COLUMN 없음
--   - CREATE OR REPLACE FUNCTION 만 사용
-- =============================================================================
...
comment on function public.transition_quote_to_contract(
  ...
  'Bundle 1: 견적→계약·현장·실행예산 draft 원자 전환. 허용 status=발송완료. p_project_mode=link|create.';
...
begin
  if to_regprocedure(
    ...
    raise exception 'Migration 39 verify failed: RPC missing';
  end if;
  ...
  if v_src is null
     or position('''발송완료''' in v_src) = 0
     or position('발송완료 상태의 견적만 전환할 수 있습니다.' in v_src) = 0
     or position('''승인'', ''계약전환''' in v_src) > 0 then
    raise exception
      'Migration 39 verify failed: status guard not updated to 발송완료';
  end if;
end;
$$;
