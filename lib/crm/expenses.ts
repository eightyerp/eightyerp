...
export async function listSettlementAdjustments(
  limit = 200,
): Promise<SettlementAdjustmentRecord[]> {
  await getExpenseAccess();
  const supabase = await createClient();
  const { data, error } = await supabase
...
export const findOrCreateVendorCandidate = (input: {
  name: string;
  businessNumber?: string | null;
  phone?: string | null;
  createdFrom?: string;
}) =>
  rpc<{ vendor_id: string; created: boolean; review_status: string; name: string }>(
    "find_or_create_vendor_candidate",
    {
      p_name: input.name,
...
export const approveVendor = (vendorId: string) =>
  rpc("approve_vendor", { p_vendor_id: vendorId });

export const registerExpenseRequest = (input: Record<string, unknown>) =>
  rpc<{
    expense_id: string;
    status: string;
    project_id: string;
...
export const approveExpenseRequest = (expenseId: string) =>
  rpc<Record<string, unknown>>("approve_expense_request", {
    p_expense_id: expenseId,
  });

export const rejectExpenseRequest = (expenseId: string, reason: string) =>
  rpc<Record<string, unknown>>("reject_expense_request", {
    p_expense_id: expenseId,
    p_reason: reason,
  });
...
export const markExpensePaid = (
  ...
  rpc<Record<string, unknown>>("mark_expense_paid", {
    p_expense_id: expenseId,
    p_paid_at: paidAt,
    p_payment_method: paymentMethod,
  });

export const cancelExpenseRequest = (expenseId: string, reason: string) =>
  rpc<Record<string, unknown>>("cancel_expense_request", {
    p_expense_id: expenseId,
    p_reason: reason,
  });

export const checkExpenseDocumentDuplicate = (sha256: string) =>
  rpc<boolean>("check_expense_document_duplicate", { p_sha256: sha256 });

export const setExpenseTaxEvidence = (input: {
  expenseId: string;
  taxEvidenceType: ExpenseTaxEvidenceType;
  supplyAmount: number;
  vatAmount: number;
  totalAmount: number;
}) =>
  ...
  }>("set_expense_tax_evidence", {
    p_expense_id: input.expenseId,
    p_tax_evidence_type: input.taxEvidenceType,
    p_supply_amount: input.supplyAmount,
    p_vat_amount: input.vatAmount,
    p_total_amount: input.totalAmount,
  });
...
export const attachExpenseDocument = (input: Record<string, unknown>) =>
  rpc("attach_expense_document", {
    p_expense_id: input.expenseId,
...
export async function createExpenseDocumentSignedUrl(
  document: Pick<ExpenseDocumentRecord, "storage_path">,
) {
  await getExpenseAccess();
  const supabase = await createClient();
...
export async function listMyExpenseNotifications(
  limit = 10,
): Promise<ExpenseNotificationItem[]> {
  const access = await getExpenseAccess();
  const supabase = await createClient();
  let query = supabase
  ...
  if (access.isFinanceAdmin) {
    query = query.eq("event_type", "expense_requested");
  } else {
    if (!access.currentEmployeeId) return [];
    query = query
      .in("event_type", ["expense_approved", "expense_paid"])
      .contains("payload", {
  ...
  return (data ?? []).map((row) => {
    ...
    return {
      ...
      requesterEmployeeId: p.requester_employee_id
        ? String(p.requester_employee_id)
        : null,
