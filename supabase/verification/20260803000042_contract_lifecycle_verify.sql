...
begin
  ...
  -- 2) 임시 고객
  -- 실제 customers 컬럼: name/phone NOT NULL, status enum(기본 '신규'),
  -- consultation_type 기본 '기타', company_id 기본 current_company_id(),
  -- assigned_employee_id FK. created_by/updated_by 없음.
  ...
  -- 3) 임시 현장 (projects: created_by/updated_by/company_id 존재)
  ...
  -- 5) 초안 수정 (update_contract_draft(uuid, jsonb))
  v_supply := 20000000;
  v_vat := 2000000;
  v_discount := 1000000;
  v_amount := v_supply - v_discount + v_vat; -- 21000000
  ...
  v_vat := 3000000;
  v_discount := 0;
  v_amount := v_supply - v_discount + v_vat; -- 33000000
  ...
  -- Migration 41: 변경 확정 시 display contract_amount만 갱신, supply/vat/discount 스냅샷 유지
  ...
  -- 8) 추가계약 (create_contract_addition — payload = 추가분 금액)
  v_supply := 5000000;
  v_vat := 500000;
  v_discount := 0;
  v_amount := v_supply - v_discount + v_vat; -- 5500000
  ...
  -- 9) 해지 (terminate_contract(uuid, jsonb) — keys: reason/fault/memo/*_amount)
  ...
  -- 10) 관리자 복구 (restore_terminated_contract(uuid, text))
  ...
  -- Migration 41: halted 예산은 status='confirmed', halted_at/halt_reason null 로 복구
  select * into v_budget from public.execution_budgets where id = v_budget_id;
