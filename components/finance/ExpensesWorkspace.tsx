  ...
  function applyAnalysis(next: ExpenseDocumentAnalysis) {
    ...
    if (next.vendorName) {
      ...
      const found = vendors.find((vendor) =>
        vendor.normalized_name === norm || normalizeVendor(vendor.name) === norm,
      );
  ...
  async function analyzeDocument() {
    ...
    try {
      ...
      if (result.success && result.analysis) {
        ...
        const warning = result.analysis.warnings.length > 0
          ? ` 자동인식 완료 · 확인필요: ${result.analysis.warnings.join(" / ")}`
          : " 자동인식 완료. 금액과 거래처를 확인 후 제출하세요.";
        setAnalysisMessage(warning.trim());
      } else {
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs leading-relaxed text-slate-600">
              현장 선택은 필수입니다. 영수증·거래명세서를 첨부하면 AI가 지출 초안을 채우며, 자동값은 반드시 확인 후 제출하세요.
            </p>
          </div>
        ...
        {projects.length === 0 ? (
          ...
          <form action={action} className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              ...
              <select name="project_id" required value={projectId} onChange={(e) => setProjectId(e.target.value)} className="min-h-11 w-full rounded-lg border border-gray-300 bg-white px-3 text-sm">
                ...
                {projects.map((project) => (
                  <option key={project.id} value={project.id}>
                    {project.customers?.name ?? "고객"} · {project.name}{project.address ? ` · ${project.address}` : ""}
                  </option>
                ))}
              </select>
            </label>
            ...
            <div className="sm:col-span-2 xl:col-span-3 rounded-xl border border-sky-200 bg-sky-50/60 p-4">
              <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                ...
                <label className="sm:col-span-1 xl:col-span-2">
                  ...
                  <input ref={fileRef} name="document" type="file" accept="image/jpeg,image/png,image/webp,application/pdf" className="block min-h-11 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm" />
                </label>
                <div className="flex items-end">
                  <button type="button" onClick={analyzeDocument} disabled={analysisBusy} className="min-h-11 w-full rounded-lg bg-sky-700 px-4 text-sm font-bold text-white hover:bg-sky-800 disabled:opacity-50">
                    {analysisBusy ? "자동인식 중..." : "증빙 자동인식"}
                  </button>
                </div>
              </div>
              <p className="mt-2 text-xs text-slate-600">같은 파일은 중복 등록을 차단합니다. PDF 거래명세서도 지원합니다.</p>
            </div>
            ...
            <label>
              ...
              {vendorName && !vendorId ? <span className="mt-1 block text-[11px] font-medium text-amber-700">기존 거래처가 없으면 신규 거래처 후보로 함께 등록됩니다.</span> : null}
            </label>

            <label>
              <span className="mb-1.5 block text-xs font-semibold text-slate-700">사업자번호</span>
              <input name="business_number" value={businessNumber} onChange={(e) => setBusinessNumber(e.target.value)} className="min-h-11 w-full rounded-lg border border-gray-300 px-3 text-sm" />
            </label>
            <label>
              <span className="mb-1.5 block text-xs font-semibold text-slate-700">거래처 연락처</span>
              <input name="vendor_phone" value={vendorPhone} onChange={(e) => setVendorPhone(e.target.value)} className="min-h-11 w-full rounded-lg border border-gray-300 px-3 text-sm" />
            </label>
            ...
            <label>
              <span className="mb-1.5 block text-xs font-semibold text-slate-700">지출일</span>
              <input name="expense_date" type="date" value={expenseDate} onChange={(e) => setExpenseDate(e.target.value)} required className="min-h-11 w-full rounded-lg border border-gray-300 px-3 text-sm" />
            </label>

            <label>
              <span className="mb-1.5 block text-xs font-semibold text-slate-700">공급가</span>
              <input name="supply_amount" type="number" min="0" step="1" value={supplyAmount} onChange={(e) => setSupplyAmount(Number(e.target.value) || 0)} required className="min-h-11 w-full rounded-lg border border-gray-300 px-3 text-sm" />
            </label>
            <label>
              <span className="mb-1.5 block text-xs font-semibold text-slate-700">부가세</span>
              <input name="vat_amount" type="number" min="0" step="1" value={vatAmount} onChange={(e) => setVatAmount(Number(e.target.value) || 0)} required className="min-h-11 w-full rounded-lg border border-gray-300 px-3 text-sm" />
            </label>
            <label>
              <span className="mb-1.5 block text-xs font-semibold text-slate-700">합계</span>
              <input name="total_amount" type="number" min="1" step="1" value={totalAmount} onChange={(e) => setTotalAmount(Number(e.target.value) || 0)} required className={`min-h-11 w-full rounded-lg border px-3 text-sm font-bold ${supplyAmount + vatAmount === totalAmount ? "border-gray-300" : "border-red-400 bg-red-50"}`} />
              {supplyAmount + vatAmount !== totalAmount ? <span className="mt-1 block text-[11px] font-bold text-red-700">공급가 + 부가세가 합계와 다릅니다.</span> : null}
            </label>

            <label className="sm:col-span-2">
              <span className="mb-1.5 block text-xs font-semibold text-slate-700">지출내용</span>
              <input name="description" value={description} onChange={(e) => setDescription(e.target.value)} required placeholder="예: 현관 타일 자재 구입" className="min-h-11 w-full rounded-lg border border-gray-300 px-3 text-sm" />
            </label>
            <label>
              <span className="mb-1.5 block text-xs font-semibold text-slate-700">지급예정일</span>
              <input name="payment_due_date" type="date" className="min-h-11 w-full rounded-lg border border-gray-300 px-3 text-sm" />
            </label>
            <label className="sm:col-span-2">
              <span className="mb-1.5 block text-xs font-semibold text-slate-700">메모</span>
              <input name="memo" placeholder="추가 확인사항" className="min-h-11 w-full rounded-lg border border-gray-300 px-3 text-sm" />
            </label>
            <div className="flex items-end">
              ...
              <button type="submit" disabled={submitting || !projectId || supplyAmount + vatAmount !== totalAmount} className="min-h-11 w-full rounded-lg bg-navy-900 px-4 text-sm font-bold text-white hover:bg-navy-800 disabled:cursor-not-allowed disabled:opacity-50">
      ...
      {isFinanceAdmin && pendingVendors.length > 0 ? (
        <section className="rounded-xl border border-violet-200 bg-violet-50/60 p-4 sm:p-5">
          ...
          <p className="mt-1 text-xs text-violet-800">직원이 증빙에서 등록한 신규 거래처 후보입니다.</p>
      ...
      {isFinanceAdmin ? (
        <section className="rounded-xl border border-amber-200 bg-amber-50/60 p-4 sm:p-5">
          <div className="flex items-center justify-between gap-3">
            <div><h2 className="font-bold text-amber-950">지출 승인대기</h2><p className="mt-1 text-xs text-amber-800">증빙과 현장을 확인한 뒤 승인 또는 반려하세요.</p></div>
          ...
          {pendingRequests.length === 0 ? <p className="mt-4 rounded-lg bg-white/70 px-4 py-5 text-center text-sm text-amber-900">승인대기 지출이 없습니다.</p> : (
            <div className="mt-4 space-y-3">
              {pendingRequests.map((row) => (
                <div key={row.id} className="rounded-xl border border-amber-200 bg-white p-4">
          ...
          <div className="mt-3 space-y-3">
            {approvedRequests.map((row) => (
              <div key={row.id} className="flex flex-col gap-3 rounded-lg border border-emerald-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
                <div><p className="font-bold text-slate-950">{row.projects?.name ?? "현장"} · {row.vendor_name_snapshot ?? "거래처"} · {money(row.total_amount)}</p><p className="mt-1 text-xs text-slate-600">{row.description}</p></div>
      ...
      <section className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="border-b border-gray-100 px-4 py-4 sm:px-5"><h2 className="font-bold text-slate-950">지출 이력</h2><p className="mt-1 text-xs text-slate-600">승인·지급·반려·취소 이력을 보존합니다.</p></div>
...
function Summary({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl border border-gray-200 bg-white p-4"><p className="text-xs font-semibold text-slate-600">{label}</p><p className="mt-1 text-xl font-bold text-slate-950">{value}</p></div>;
}
