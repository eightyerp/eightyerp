...
function intMoney(value: FormDataEntryValue | null, label: string): number {
  ...
  if (!Number.isFinite(n) || !Number.isInteger(n) || n < 0) {
    throw new Error(`${label}은 원 단위 정수로 입력해 주세요.`);
  }
  return n;
}
...
async function pushExpenseEvent(
  ...
  await enqueueNotificationEvent({
    ...
    recipient:
      eventType === "expense_requested"
        ? null
        : employee?.phone || employee?.email || null,
...
export async function analyzeExpenseDocumentAction(
  formData: FormData,
  ...
  try {
    ...
    if (!(file instanceof File)) {
      throw new Error("영수증 또는 거래명세서를 선택해 주세요.");
    }
    assertDocument(file);
    ...
    if (await checkExpenseDocumentDuplicate(digest)) {
      return {
        success: false,
        duplicate: true,
        error: "같은 증빙 파일이 이미 등록되어 있습니다. 중복 지출 여부를 확인해 주세요.",
      };
    }

    const ai = await analyzeExpenseDocumentWithAi(file);
    if (!ai.available) {
      return { success: false, duplicate: false, error: ai.error };
    }
...
export async function registerExpenseRequestAction(
  ...
  try {
    ...
    if (totalAmount <= 0 || supplyAmount + vatAmount !== totalAmount) {
      throw new Error("공급가 + 부가세 = 합계가 되도록 금액을 확인해 주세요.");
    }

    const expenseDate = String(formData.get("expense_date") ?? "").trim();
    if (!/^\d{4}-\d{2}-\d{2}$/.test(expenseDate)) {
      throw new Error("지출일을 선택해 주세요.");
    }
    ...
    const vendorPhone =
      String(formData.get("vendor_phone") ?? "").trim() || null;
    ...
    if (hasDocument && file instanceof File) {
      ...
      if (await checkExpenseDocumentDuplicate(digest)) {
        throw new Error(
          "같은 증빙 파일이 이미 등록되어 있습니다. 중복 지출 여부를 확인해 주세요.",
        );
      }
    }
    ...
    const memoBase = String(formData.get("memo") ?? "").trim();
    const evidenceMemo = !hasDocument
      ? "증빙 미첨부 - 추후 영수증/카드내역/문자 등으로 보완 가능"
      : "";
    const memo = [memoBase, evidenceMemo].filter(Boolean).join(" / ") || null;
    ...
    if (hasDocument && file instanceof File && uploadedPath && digest) {
      ...
      try {
        aiExtracted = JSON.parse(
          String(formData.get("ai_extracted") ?? "{}"),
    ...
    if (result.status === "pending") {
      await pushExpenseEvent("expense_requested", result.expense_id);
    } else {
      await pushExpenseEvent("expense_approved", result.expense_id);
    }

    revalidatePath("/finance/payments");
    revalidatePath("/dashboard");

    const evidenceText = hasDocument ? "증빙 포함" : "증빙 미첨부";
    return {
      success: true,
      expenseId: result.expense_id,
      message:
        result.status === "pending"
          ? `지출요청을 ${evidenceText} 상태로 등록하고 관리자에게 PUSH했습니다.`
          : `관리자 지출을 ${evidenceText} 상태로 등록했습니다.`,
    };
  } catch (error) {
    if (createdExpenseId) {
      try {
    ...
    return {
      success: false,
      error:
        error instanceof Error ? error.message : "지출요청 등록에 실패했습니다.",
    };
  }
}

export async function attachExpenseDocumentLaterAction(
  expenseId: string,
  formData: FormData,
): Promise<ExpenseActionResult> {
  let uploadedPath: string | null = null;
  ...
  try {
    ...
    if (!normalizedExpenseId) throw new Error("지출요청 정보가 없습니다.");

    const file = formData.get("document");
    if (!(file instanceof File) || file.size <= 0) {
      throw new Error("추가할 증빙 파일을 선택해 주세요.");
    }
    assertDocument(file);
    ...
    await attachExpenseDocument({
      ...
      documentType: documentType(formData.get("document_type")),
      storagePath: uploadedPath,
      originalFileName: file.name || "추가 증빙",
      mimeType: file.type || "",
      fileSize: file.size,
      sha256: digest,
      aiExtracted: {},
    ...
    revalidatePath("/finance/payments");
    return { success: true, message: "증빙을 추가했습니다." };
  } catch (error) {
    if (uploadedPath) {
      try {
        const supabase = await createClient();
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : "증빙 추가에 실패했습니다.",
    };
  }
}
...
export async function approveExpenseRequestAction(
  ...
  try {
    ...
    return {
      success: true,
      message: "지출요청을 승인하고 신청자에게 PUSH했습니다.",
    };
  } catch (error) {
    return {
      success: false,
...
export async function resolveAndApprovePostSettlementExpenseAction(
  ...
  try {
    ...
    const adjustmentAmount = safeAdjustmentMoney(
      input.adjustmentAmount,
      "다음 정산 차감액",
    );
    const recoveryExpectedAmount = safeAdjustmentMoney(
      input.recoveryExpectedAmount,
      "회수 예정액",
    );
    ...
    return {
      success: true,
      message: "사후지출 처리방법을 저장하고 승인했습니다.",
    };
  } catch (error) {
    return {
      success: false,
      error:
        error instanceof Error ? error.message : "사후지출 승인에 실패했습니다.",
    };
  }
}
...
export async function rejectExpenseRequestAction(
  ...
  try {
    await rejectExpenseRequest(expenseId, reason);
    revalidatePath("/finance/payments");
    return { success: true, message: "지출요청을 반려했습니다." };
  } catch (error) {
    return {
      success: false,
...
export async function markExpensePaidAction(
  ...
  try {
    ...
    return {
      success: true,
      message: "지급완료 처리하고 신청자에게 PUSH했습니다.",
    };
  } catch (error) {
    return {
      success: false,
...
export async function cancelExpenseRequestAction(
  expenseId: string,
  reason: string,
): Promise<ExpenseActionResult> {
  try {
    await cancelExpenseRequest(expenseId, reason);
    revalidatePath("/finance/payments");
    return { success: true, message: "지출요청을 취소했습니다." };
  } catch (error) {
    return {
      success: false,
...
export async function approveVendorAction(
  ...
  try {
    ...
    revalidatePath("/finance/payments");
    return { success: true, message: "신규 거래처를 승인했습니다." };
  } catch (error) {
    return {
      success: false,
      error:
        error instanceof Error ? error.message : "거래처 승인에 실패했습니다.",
    };
  }
}

export async function getMyExpenseNotificationsAction(): Promise<
  ExpenseNotificationItem[]
> {
  try {
    return await listMyExpenseNotifications(10);
  } catch {
    return [];
  }
}
