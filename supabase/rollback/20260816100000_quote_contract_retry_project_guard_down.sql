-- =============================================================================
-- Emergency rollback for Migration 20260816100000
--
-- migration 39의 RPC 정의를 정확히 복원한다. 이 롤백은 다른 project_id 재시도를
-- 다시 멱등 성공으로 받아들이는 기존 결함을 의도적으로 재개방하므로,
-- 앱/DB 긴급 복구 승인 때만 사용한다. 테이블/데이터 변경 없음.
-- =============================================================================
...
begin
  ...
  if v_src is null
     or position('contract replay project mismatch' in v_src) > 0
     or position('발송완료 상태의 견적만 전환할 수 있습니다.' in v_src) = 0 then
    raise exception
      'Rollback 20260816100000 verify failed: migration 39 function not restored';
  end if;
end;
$verify$;
