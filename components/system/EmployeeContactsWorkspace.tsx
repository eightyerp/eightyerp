    ...
    <section className="dashboard-card overflow-hidden">
      <div className="flex items-center justify-between border-b border-slate-200 p-5">
        <div>
          ...
          <p className="text-xs text-slate-500">총 {visibleEmployees.length}명</p>
        </div>
        {canManageAll ? (
              ...
              return (
                <tr key={employee.id} className="border-t border-slate-100">
                  <td className="px-4 py-3">
                    ...
                    <p className={`text-xs ${employee.is_active ? "text-emerald-600" : "text-slate-400"}`}>{employee.merged_into_employee_id ? "병합됨" : employee.is_active ? "활성" : "비활성"}</p>
                  </td>
                  <td className="px-4 py-3">{team?.name ?? "-"} · {employee.title}</td>
                  <td className="px-4 py-3"><p>{employee.phone ?? "-"}</p><p className="text-xs text-slate-500">{employee.email ?? "-"}</p></td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-xl bg-white p-5 shadow-xl">
            ...
            <form action={(formData) => run(() => saveEmployeeMasterAction(formData), "직원 Master를 저장했습니다." )} className="mt-4 grid gap-3 sm:grid-cols-2">
              ...
              <label className="text-sm">이름<input name="name" required defaultValue={selected?.name ?? ""} className="mt-1 w-full rounded-lg border px-3 py-2" /></label>
              <label className="text-sm">팀<select name="team_id" defaultValue={selected?.team_id ?? ""} className="mt-1 w-full rounded-lg border px-3 py-2"><option value="">미지정</option>{teams.map((team) => <option key={team.id} value={team.id}>{team.name}</option>)}</select></label>
              <label className="text-sm">직책<input name="title" required defaultValue={selected?.title ?? ""} className="mt-1 w-full rounded-lg border px-3 py-2" /></label>
              <label className="text-sm">전화<input name="phone" defaultValue={selected?.phone ?? ""} className="mt-1 w-full rounded-lg border px-3 py-2" /></label>
              <label className="text-sm sm:col-span-2">이메일<input name="email" type="email" defaultValue={selected?.email ?? ""} className="mt-1 w-full rounded-lg border px-3 py-2" /></label>
              {selected ? <label className="text-sm">상태<select name="is_active" defaultValue={String(selected.is_active)} className="mt-1 w-full rounded-lg border px-3 py-2"><option value="true">활성</option><option value="false">비활성</option></select></label> : null}
              {selected?.login_linked ? <label className="text-sm">권한<select name="login_role" defaultValue={selected.role ?? "staff"} className="mt-1 w-full rounded-lg border px-3 py-2"><option value="staff">직원</option><option value="manager">팀장</option><option value="admin">관리자</option><option value="super_admin">최고 관리자</option></select></label> : null}
              <div className="flex flex-wrap gap-2 sm:col-span-2">
                <button type="submit" disabled={pending} className="rounded-lg bg-navy-800 px-4 py-2 text-sm text-white">저장</button>
                <button type="button" onClick={() => { setSelected(null); setCreating(false); }} className="rounded-lg border px-4 py-2 text-sm">닫기</button>
              </div>
            </form>
              ...
              <div className="mt-5 border-t pt-4">
                ...
                {selected.login_linked ? (
                  ...
                  <form action={(formData) => run(() => linkEmployeeLoginAction(formData), "로그인 계정을 연결했습니다.")} className="mt-2 flex flex-wrap gap-2">
                    <input type="hidden" name="employee_id" value={selected.id} />
                    <select name="user_id" required className="min-w-56 rounded-lg border px-3 py-2 text-sm"><option value="">가입 계정 선택</option>{pendingAccounts.map((profile) => <option key={profile.id} value={profile.id}>{profile.full_name ?? profile.email ?? profile.id} · {profile.email}</option>)}</select>
                    <select name="role" defaultValue="staff" className="rounded-lg border px-3 py-2 text-sm"><option value="staff">직원</option><option value="manager">팀장</option><option value="admin">관리자</option></select>
                    <button type="submit" disabled={pending} className="rounded-lg bg-emerald-600 px-3 py-2 text-sm text-white">계정 연결</button>
                  </form>
                ) : <p className="mt-2 text-sm text-slate-500">연결 가능한 가입 대기 계정이 없습니다.</p>}
              </div>
            ) : null}
            {selected && canManageAll ? (
              <div className="mt-5 border-t pt-4">
                ...
                {selected.customer_count + selected.quote_count + selected.schedule_count > 0 ? (
                  <form action={(formData) => {
                    ...
                    <select name="target_employee_id" required className="min-w-56 rounded-lg border px-3 py-2 text-sm">
                    ...
                    <button type="submit" disabled={pending} className="rounded-lg bg-amber-600 px-3 py-2 text-sm text-white">담당업무 일괄 이전</button>
                  </form>
            ...
            {selected?.role ? (
              <div className="mt-5 border-t pt-4">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  ...
                  ).map((menu) => <span key={menu} className="rounded-full bg-slate-100 px-3 py-1 text-xs text-slate-700">{menu}</span>)}
                </div>
              </div>
            ) : null}
            {selected ? (
              <div className="mt-5 border-t pt-4">
                ...
                <div className="mt-2 max-h-44 space-y-2 overflow-y-auto">
                  {events.filter((event) => event.employee_id === selected.id).map((event) => <div key={event.id} className="rounded-lg bg-slate-50 p-2 text-xs"><span className="font-medium">{event.event_type}</span><span className="ml-2 text-slate-500">{formatDateTime(event.created_at)}</span></div>)}
                  {events.every((event) => event.employee_id !== selected.id) ? <p className="text-xs text-slate-500">기록이 없습니다.</p> : null}
                </div>
              </div>
            ) : null}
          ...
          <div className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-xl bg-white p-5 shadow-xl">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <label className="text-sm">기준 직원(유지)
                <select value={mergeTargetId} onChange={(e) => { setMergeTargetId(e.target.value); setMergeImpact(null); }} className="mt-1 w-full rounded-lg border px-3 py-2">
              ...
              <label className="text-sm">중복 직원(병합)
                <select value={mergeSourceId} onChange={(e) => { setMergeSourceId(e.target.value); setMergeImpact(null); }} className="mt-1 w-full rounded-lg border px-3 py-2">
                  <option value="">선택</option>
            ...
            }} className="mt-3 rounded-lg bg-slate-800 px-4 py-2 text-sm text-white disabled:opacity-50">병합 영향 분석</button>
              ...
              return <div className="mt-5 space-y-4 border-t pt-4">
                ...
                <div>
                  ...
                  <div className="mt-2 max-h-48 overflow-y-auto rounded-lg border">
                    <table className="min-w-full text-xs"><thead className="bg-slate-50"><tr><th className="p-2 text-left">테이블.컬럼</th><th>구분</th><th>이전</th><th>병합 후 합계</th></tr></thead>
                ...
                <div>
                  ...
                  {mergeImpact.logins.length === 0 ? <p className="mt-1 text-sm text-slate-500">연결 계정 없음</p> : <div className="mt-2 space-y-2">{mergeImpact.logins.map((login) => <label key={login.profile_id} className="flex items-center gap-2 rounded-lg border p-2 text-sm"><input type="radio" name="keep_profile" checked={keepProfileId === login.profile_id} onChange={() => setKeepProfileId(login.profile_id)} />{login.email ?? login.full_name ?? login.profile_id} · {login.employee_id === mergeImpact.target.id ? "기준 직원 계정" : "중복 직원 계정"}</label>)}</div>}
                  {bothHaveLogin ? <label className="mt-2 block text-sm">나머지 계정 처리<select value={otherLoginAction} onChange={(e) => setOtherLoginAction(e.target.value as "unlink" | "deactivate")} className="ml-2 rounded-lg border px-2 py-1"><option value="unlink">직원 연결만 해제</option><option value="deactivate">연결 해제 및 비활성화</option></select></label> : null}
                </div>
                ...
                }} className="rounded-lg bg-red-700 px-4 py-2 text-sm text-white disabled:opacity-50">확인 후 직원 병합</button>
              </div>;
            })() : null}
            {mergeReport ? (() => {
              ...
              return <div className="mt-5 space-y-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4">
                ...
                <dl className="grid gap-2 text-sm sm:grid-cols-2">
                  <div><dt className="text-slate-500">병합 로그 ID</dt><dd className="break-all font-mono text-xs">{mergeReport.merge_log_id}</dd></div>
                  <div><dt className="text-slate-500">총 이전 참조</dt><dd className="font-semibold">{total}건</dd></div>
                  <div><dt className="text-slate-500">중복 직원 ID</dt><dd className="break-all font-mono text-xs">{mergeReport.source_employee_id}</dd></div>
                  <div><dt className="text-slate-500">기준 직원 ID</dt><dd className="break-all font-mono text-xs">{mergeReport.target_employee_id}</dd></div>
                </dl>
            ...
            {error ? <p className="mt-3 rounded-lg bg-red-50 px-3 py-2 text-sm text-red-700">{error}</p> : null}
            <button type="button" onClick={() => { setMergeOpen(false); setMergeImpact(null); setError(null); }} className="mt-4 rounded-lg border px-4 py-2 text-sm">닫기</button>
          </div>
        </div>
      ) : null}
    </section>
